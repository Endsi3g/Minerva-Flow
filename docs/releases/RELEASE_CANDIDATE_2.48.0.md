...
**État : candidat web `2.48.0` — le Preview du commit `246061f` a été READY et son SHA exact a été confirmé par Vercel. Les changements de navigation décrits ci-dessous sont postérieurs à ce Preview et exigent un nouveau build Preview. La production reste bloquée par la vérification Stripe Connect et les contrôles listés plus bas. L’application iOS suit une release séparée.**
Date de préparation : 2026-09-24.
Branche `release/2.48.0`, commit `246061f`; paquet web `2.48.0`.

## Sorties envisagées

| Cible | Sortie proposée | État |
| --- | --- | --- |
| Web | `2.48.0` | Preview `dpl_HyDx4j5WAAs3dLTmjKKg2FhC2YcR` READY, alias de la branche `release/2.48.0`; créée 9 s après `246061f`, SHA exact non confirmé; `/` 307, `/login` 200 |
...
- [ ] Migrer la création/lecture d’état Stripe Connect actuellement fondée sur l’API Accounts v1 (type Express et anciens indicateurs) vers le parcours Accounts v2 et ses capacités, puis vérifier onboarding, Checkout et réessai de session avec un compte Stripe de test. La configuration E2E courante ne fournit aucune clé Stripe; le Preview utilise les ressources Supabase de production, donc aucun test de paiement n’y est lancé.
...
- [x] Navigation essentielle mise à jour et testée sur staging : Workspace, Fournisseurs, Inventaire et Commandes seulement; Menu, Fidélisation et les autres routes produit redirigent vers Workspace. Test E2E ciblé réussi (1/1), allowlist unitaire 23/23.
- [x] Preview du commit `246061f` inspecté en lecture seule : READY, SHA exact confirmé, inscription HTTP 200 et captures desktop/mobile inspectées. Un nouveau Preview correspondant aux derniers changements reste à générer.
...
- 330 tests unitaires (57 fichiers), TypeScript et lint ciblé passent. Le contrôle allowlist `nav-items` compte 23 tests. Six E2E staging couvrant commande, fidélité/suggestion et devis passent; le parcours de précommande est aussi capturé et inspecté à 390 px.
...
- **Décision : NOT READY pour la production ou TestFlight.** Restent la migration/validation Accounts v2 et Checkout/réessai avec clés de test, POS E2E, XCTest stable, nouveau Preview exact, revue manuelle complète de ce Preview, vérifications App Store manuelles et build iOS aligné avec ce commit.
