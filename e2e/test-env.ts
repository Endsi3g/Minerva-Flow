...
function configureE2eEnvironment() {
  ...
  const stripePublishableTestKey = process.env.E2E_TEST_STRIPE_PUBLISHABLE_KEY?.trim() || "";
  if (stripePublishableTestKey && !stripePublishableTestKey.startsWith("pk_test_")) {
    throw new Error("E2E_TEST_STRIPE_PUBLISHABLE_KEY must be a Stripe test-mode key (pk_test_…).");
  }
  ...
  // Never let a developer's .env.local inject a live Stripe key into the
  // browser bundle generated by the local Next.js E2E server.
  process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY = stripePublishableTestKey;
