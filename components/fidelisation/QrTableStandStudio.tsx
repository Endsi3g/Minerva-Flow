...
export function QrTableStandStudio({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Background
      ctx.fillStyle = theme === "cream" ? "#FDFBF7" : theme === "ink" ? "#111827" : theme === "gold" ? "#1E1B18" : "#062419";
      ctx.fillRect(0, 0, width, height);

      // Border outline
      ctx.strokeStyle = theme === "cream" ? "#E5DFD5" : "rgba(255,255,255,0.12)";
      ctx.lineWidth = 16;
  ...
  return (
    <Modal
      ...
      description="Créez et téléchargez des supports physiques élégants pour vos tables et comptoirs."
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-2">
        ...
        <div className="lg:col-span-6 flex flex-col items-center justify-center">
          ...
          <div
            className={`w-full max-w-[280px] rounded-2xl p-5 border shadow-xl transition-all ${activeTheme.bg} ${activeTheme.text} ${activeTheme.border} ${
          ...
          >
            <div>
              ...
              <h4 className="mt-1 font-serif text-[15px] font-bold leading-tight line-clamp-2">{headline}</h4>
              <p className="mt-1 text-[9.5px] opacity-75 line-clamp-2">{subline}</p>
            </div>

            <div className="my-auto rounded-xl bg-white p-2 shadow-md">
              {qrDataUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={qrDataUrl} alt="QR Code de table" className="h-28 w-28 object-contain" />
              ) : (
            ...
            <p className={`text-[9px] font-semibold tracking-wider ${activeTheme.accent}`}>✦ MINERVA FLOW VIP ✦</p>
          </div>
        </div>
      </div>
    </Modal>
