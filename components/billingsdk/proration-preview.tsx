      ...
      <Card className={cn(cardVariants({ theme }))}>
        <CardHeader className="px-4 pb-3 sm:px-6 sm:pb-4">
          <CardTitle className="flex items-center gap-2 text-base sm:text-lg">
            ...
            Aperçu du changement de forfait
          </CardTitle>
          <p className="text-muted-foreground text-sm">
            Vérifiez les frais et crédits liés à votre changement de forfait
          </p>
        </CardHeader>
        ...
        <CardContent className="space-y-4 px-4 sm:space-y-6 sm:px-6">
          ...
          <div className="grid grid-cols-1 items-center gap-3 sm:gap-4 md:grid-cols-3">
            ...
            >
              ...
              <p className="text-muted-foreground mb-3 text-xs sm:text-sm">
                {isCustomCurrent
                  ? "Sur devis"
                  : `${creditCurrency}${currentPrice}/${currentPlan.type === "yearly" ? "an" : "mois"}`}
              </p>
            ...
            >
              ...
              <p className="text-muted-foreground mb-3 text-xs sm:text-sm">
                {isCustomNew
                  ? "Sur devis"
                  : `${chargeCurrency}${newPrice}/${billingCycle === "yearly" ? "an" : "mois"}`}
              </p>
              <div className="text-muted-foreground flex items-center gap-1 text-xs">
                <Calendar className="h-3 w-3 sm:h-4 sm:w-4" />
                Effectif {effectiveDate === "immediately" ? "immédiatement" : effectiveDate}
              </div>
            </motion.div>
          </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs sm:text-sm">
                <span className="text-muted-foreground">
                  Crédit pour le temps non utilisé
                </span>
              ...
              <div className="flex items-center justify-between text-xs sm:text-sm">
                <span className="text-muted-foreground">
                  Frais proratisés ({prorationDays} jours)
                </span>
              ...
              <div className="flex items-center justify-between font-semibold">
                <span>
                  {canCompute
                    ...
                    : "Le montant sera calculé au moment du paiement"}
                </span>
                <span
                  className={cn(
          ...
          >
            <p className="text-muted-foreground text-xs sm:text-sm">
              Votre forfait changera {effectiveDate === "immediately" ? "immédiatement" : effectiveDate}.
              {isNextCycle
                ? " Aucun frais immédiat."
                : hasComparablePrices
                  ? netAmount >= 0
                    ? ` Vous serez facturé ${chargeCurrency}${Math.abs(netAmount).toFixed(2)}.`
                    : ` Vous recevrez un crédit de ${chargeCurrency}${Math.abs(netAmount).toFixed(2)}.`
                  : " Le montant sera calculé au moment du paiement."}
            </p>
          </motion.div>
