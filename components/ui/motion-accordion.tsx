...
function AccordionItem({
  ...
  return (
    <motion.div
      layout
      className={cn(
        "overflow-hidden rounded-[30px] bg-mv-surface text-mv-ink shadow-mv-sm border border-mv-border",
        isOpen && "border-mv-green/40 shadow-mv-md"
      )}
    ...
    >
      ...
      >
        <span className="font-heading text-[clamp(1.1rem,1.5vw,1.25rem)] font-normal tracking-tight leading-snug text-mv-ink">
          {item.question}
        </span>
        ...
        <motion.span
          ...
          className="inline-flex size-12 shrink-0 items-center justify-center text-mv-ink"
        >
          {isOpen ? (
            <svg
              width="14"
      ...
      >
        <motion.div
          ...
          className="px-7 pb-7"
        >
          <div className="text-base leading-7 font-normal tracking-normal text-mv-ink-soft">
            {item.answer}
          </div>
        </motion.div>
      </motion.div>
