...
- Chromium est opérationnel via le binaire headless Playwright 1.63 déjà présent dans le cache; captures visuelles login Preview 1440×900 et 390×844 : [`desktop`](docs/screenshots/preview-login-2026-09-25-desktop.png), [`mobile`](docs/screenshots/preview-login-2026-09-25-mobile.png). Aucun overflow ni `pageerror`; `/en/login` affiche néanmoins le libellé français.
...
- Inspection production read-only : `restaurants.delivery_enabled` et `orders.delivery_address` sont présentes, mais les quatre colonnes ajoutées par 0150 sont absentes. Aucune migration ni promotion production n’a été exécutée.
