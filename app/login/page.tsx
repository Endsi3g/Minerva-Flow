...
function DoorIllustration() {
  return (
    <svg width="180" height="220" viewBox="0 0 180 220" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path
        d="M120 16 C120 16 158 20 160 100 C162 160 128 198 120 200 L120 16 Z"
        fill="var(--mv-ink)"
      />
      <path
...
function OAuthButton({ label, icon }: { label: string; icon: React.ReactNode }) {
  return (
    <Link
      href="/onboarding"
      className="flex h-11 flex-1 items-center justify-center gap-2 rounded-lg border border-mv-border bg-mv-surface text-[13.5px] font-semibold text-mv-ink transition-colors hover:bg-mv-cream-soft"
    >
      {icon}
      {label}
    </Link>
...
export default function LoginPage() {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-mv-cream px-6">
      <div className="mb-8">
        <LogoMark size={40} />
      </div>
      <DoorIllustration />
      <h1 className="mt-6 text-center font-display text-[28px] font-medium leading-tight text-mv-ink">
        Se connecter ou créer
        <br />
        un compte
      </h1>

      <div className="mt-8 flex w-full max-w-sm flex-col gap-2.5 sm:flex-row">
        <OAuthButton
          label="Google"
          icon={
      ...
      <p className="mt-8 max-w-sm text-center text-[12px] leading-relaxed text-mv-ink-faint">
      ...
      <p className="mt-10 font-display text-[15px] font-medium text-mv-ink">Minerva Flow</p>
    </div>
  );
}
