  ...
  if (!employee) {
    return (
      <div className="mx-auto max-w-3xl w-full">
        ...
        <Card>
          <p className="text-[13px] text-mv-ink-soft">{t("noEmployeeRecord")}</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl w-full">
      <PageHeader
        ...
        description={t("subtitle", { roleTitle: employee.roleTitle })}
      />

      <div className="grid grid-cols-2 gap-3 mb-6">
        <Card className="text-center">
          <Clock size={20} className="mx-auto mb-1 text-mv-ink-soft" />
          <p className="text-[11px] font-semibold uppercase text-mv-ink-faint">{t("totalHours")}</p>
        ...
        <Card className="text-center">
          ...
          <p className="text-[11px] font-semibold uppercase text-mv-ink-faint">{t("punctuality")}</p>
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader eyebrow={t("eyebrow")} title={t("myTasks")} />
          <div className="space-y-2">
            {tasks.length === 0 ? (
              <p className="text-[13px] text-mv-ink-faint py-2">{t("noTasks")}</p>
            ) : (
              tasks.map((tk) => (
                <label
                  key={tk.id}
        ...
        <Card>
          <CardHeader eyebrow="Journal" title={t("myShifts")} />
          <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
            {shifts.length === 0 ? (
              <p className="text-[13px] text-mv-ink-faint py-2">{t("noShifts")}</p>
            ) : (
              shifts.map((s) => (
                <div
                  key={s.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-mv-green-dark">{t("hours", { count: s.hoursWorked })}</span>
        ...
        <Card>
          <CardHeader eyebrow="Revues" title={t("myReviews")} />
          <div className="space-y-3">
            {reviews.length === 0 ? (
              <p className="text-[13px] text-mv-ink-faint py-2">{t("noReviews")}</p>
            ) : (
              reviews.map((r) => (
                <div key={r.id} className="rounded-xl border border-mv-border p-4 space-y-2">
                  ...
                  {r.strengths && (
                    <p className="text-[12.5px] leading-relaxed text-mv-ink-soft">
                      <strong className="text-mv-ink">{t("strengths")}</strong> {r.strengths}
                    </p>
                  )}
                  {r.improvements && (
                    <p className="text-[12.5px] leading-relaxed text-mv-ink-soft">
                      <strong className="text-mv-ink">{t("improvements")}</strong> {r.improvements}
                    </p>
                  )}
                  {r.attributedRevenue !== null && (
                    <p className="text-[12.5px] font-medium text-mv-green-dark">
                      {t("attributedRevenue", { amount: formatCurrency(r.attributedRevenue) })}
                    </p>
                  )}
                  <div className="flex items-center justify-between border-t border-mv-border-soft pt-2 mt-2">
                    <span className="text-[11px] text-mv-ink-faint">{t("reviewedBy", { name: r.reviewerName })}</span>
                    <Link
