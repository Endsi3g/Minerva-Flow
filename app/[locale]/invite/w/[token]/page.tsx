...
import { getInviteByToken, redeemInvite } from "@/lib/data/workspace-invites";
  ...
  return (
    <Shell>
      ...
      <p className="mt-2 text-[13px] leading-relaxed text-mv-ink-soft">
        Rejoignez le workspace <span className="font-semibold text-mv-ink">{invite.workspaceName}</span> en
        tant que <span className="font-semibold text-mv-ink">{roleLabels[invite.role]}</span>
        {invite.restaurantNames.length > 0 && (
          <>
            {" "}
            pour <span className="font-semibold text-mv-ink">{invite.restaurantNames.join(", ")}</span>
          </>
        )}
        . Connectez-vous ou créez un compte pour continuer.
      </p>
      <div className="mt-5 flex flex-col gap-2">
        <Button href={`/login?wInviteToken=${token}`} className="w-full">
          Se connecter
        </Button>
        <Button href={`/sign-up?wInviteToken=${token}`} variant="secondary" className="w-full">
          Créer un compte
        </Button>
      </div>
    </Shell>
