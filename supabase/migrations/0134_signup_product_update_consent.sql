-- Preserve explicit email-update consent captured during account creation.
-- Missing metadata (OAuth and legacy accounts) remains opted out.
begin;
...
declare
  ...
  opted_in_product_updates boolean := (new.raw_user_meta_data ->> 'product_updates_opt_in') = 'true';
begin
