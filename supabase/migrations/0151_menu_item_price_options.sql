-- Persist fixed size/quantity price choices and validate the chosen option
-- atomically against the owner-managed menu row during checkout.
begin;
...
alter table public.menu_items
  drop constraint if exists menu_items_price_options_array_check;

alter table public.menu_items
  add constraint menu_items_price_options_array_check
