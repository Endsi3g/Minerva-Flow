-- Let the Stripe webhook distinguish the first quote-to-order conversion
-- from at-least-once retries, so the order and owner notification happen
-- exactly once for each paid checkout session.
begin;

create or replace function complete_service_quote_payment_once(
  p_quote_id uuid,
...
declare
  ...
  v_order_id uuid;
begin
  if p_quote_id is null or nullif(trim(p_checkout_session_id), '') is null
     or nullif(trim(p_payment_intent_id), '') is null then
    raise exception 'invalid_service_quote_payment' using errcode = '22023';
  end if;
  ...
  if v_quote.converted_order_id is not null then
    return query select v_quote.converted_order_id, false;
    return;
  end if;
...
revoke all on function complete_service_quote_payment_once(uuid, text, text) from public, anon, authenticated;
grant execute on function complete_service_quote_payment_once(uuid, text, text) to service_role;
