...
- [x] Web : branche `release/2.48.0` poussée jusqu’au commit `515312c`. Le Preview le plus récent, `dpl_DVYtvAbBgrgqZtGR4PC8QrYMUkKs`, est `READY` et construit exactement `515312c`; build/typecheck/source maps Sentry confirmés. Production reste sur `dpl_A35dLpmKawVPfxhp9rWEWSiVcRLc` (`f969e15`).
...
- **Candidate web `2.48.0`** : branche `release/2.48.0`, HEAD `515312c` (`Record changelog and production release gates`), contenant aussi le changement de formats tarifés `6621c38` et le changelog illustré `ef4c04c`. Preview le plus récent `dpl_DVYtvAbBgrgqZtGR4PC8QrYMUkKs`, `READY`, construit exactement `515312c`; build/typecheck/source maps Sentry confirmés. Smoke login, inscription, vie privée et conditions : 200; pages owner sans session redirigent vers login. Les images du changelog sont servies en PNG (200). Login desktop (1440×900) et mobile (390×844) sans overflow ni erreur JavaScript; `/en/login` affiche toutefois du texte français, à examiner. Production actuelle inspectée en lecture seule : `dpl_A35dLpmKawVPfxhp9rWEWSiVcRLc`, branche `main`, commit `f969e15`.
...
- **Production : NOT READY — aucune promotion, tag, release GitHub ni courriel utilisateur.** Les quatre champs de migration 0150 (`stripe_connect_account_api_version`, `stripe_connect_transfers_status`, `stripe_connect_recipient_payouts_status`, `stripe_connect_requirements_due_count`) restent absents. La migration 0151 `menu_item_price_options` a été appliquée directement et enregistrée en production sous la version `20260925184354`; la colonne est lisible via PostgREST. L’historique distant diverge du dépôt; ne pas lancer `supabase db push`. Les 21 brouillons Mains Magique sont toujours inactifs; aucun format n’est encore configuré dans ses données. Le checkout Stripe Connect reste non validé (`insufficient_capabilities_for_transfer`); POS E2E incomplet et le dernier staging E2E échoue avant assertion métier.
...
- Le configurateur de formats tarifés pour les propriétaires et la sélection correspondante dans le panier client sont intégrés au code web/iOS au commit `6621c38` et consignés au changelog. La migration 0151 est appliquée en production; le code reste sur la candidate Preview et les 21 articles Mains Magique demeurent des brouillons inactifs sans format tarifé configuré.
