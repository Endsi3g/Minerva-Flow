...
- Aucun commit/tag/release GitHub, email aux utilisateurs ou promotion Vercel effectué dans cette session.

### Demande de promotion web et statut Apple — 2026-09-25

- **Promotion Vercel Production non effectuée.** Stripe Connect demeure un blocage : le compte destinataire de test n’a pas les capacités de transfert requises (`restricted`), et aucune `STRIPE_SECRET_KEY` live n’est configurée dans l’environnement Production. Le checkout Connect en production ne peut donc pas être déclaré fonctionnel; ne pas promouvoir avant configuration sécurisée d’une clé live restreinte et validation d’un flux réel autorisé. Les tests POS sont reportés comme demandé, mais cela ne lève pas le blocage Stripe.
...
- **Limites restantes :** XCTest n’a aucun résultat confirmé; l’audit App Store statique reste à 0 critique/0 élevé et 2 avertissements (déclaration chiffrement widget à confirmer; contrôles manuels App Store). Aucun déploiement production, email utilisateurs, commit, tag ou release GitHub n’a été fait.
