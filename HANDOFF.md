...
- [x] Web : candidat applicatif `515312c` poussé; Preview `dpl_DVYtvAbBgrgqZtGR4PC8QrYMUkKs` `READY`. Un commit documentaire local `3c06648` enregistre la migration 0151; les changements Auth et tests de cette reprise ne sont pas encore commités. Production reste sur `dpl_A35dLpmKawVPfxhp9rWEWSiVcRLc` (`f969e15`).
...
- **Candidate web `2.48.0`** : le candidat applicatif `515312c` contient les formats tarifés (`6621c38`) et le changelog illustré (`ef4c04c`). Preview `dpl_DVYtvAbBgrgqZtGR4PC8QrYMUkKs`, `READY`. Smoke routes Auth/légales réussi; images changelog 200. L’inspection Auth la plus récente confirme maintenant la traduction anglaise et française sur bureau/mobile. Production actuelle : `dpl_A35dLpmKawVPfxhp9rWEWSiVcRLc`, branche `main`, commit `f969e15`.
