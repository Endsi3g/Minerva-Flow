...
- [x] Web : candidate commitée et poussée; le Preview exact a terminé READY et ses contrôles HTTP publics/protection Auth ont été vérifiés. La promotion production reste bloquée par les validations fonctionnelles et de schéma ci-dessous.
...
- **Candidate web `2.48.0`** : commit `20197a4c85f3aed215c6cd12ce69004b9ab70a4b`, poussé sur `release/2.48.0`. Preview exact `dpl_CaxWx94fLR97gDc3uwSrrwcnpetV`, `READY`; alias `https://minerva-flow-git-release-2480-endsi3gs-projects.vercel.app`. Build Vercel compilé, source maps Sentry téléversées. Smoke HTTP: login, inscription et pages légales retournent 200; routes owner sans session redirigent au login. Aucun E2E authentifié/écriture n’a été lancé, car l’environnement Vercel utilise Supabase production. Aucun contrôle visuel Playwright n’a pu être fait: le navigateur manquait et le téléchargement Chromium depuis le CDN était trop lent. Production n’a pas été promue et reste sur le déploiement antérieur; confirmer son SHA avant toute promotion.
...
- **Production : NOT READY — aucune promotion, tag, release GitHub ni courriel utilisateur.** Checkout/PaymentIntent n’est pas validé (compte test sans capacité de transfert, erreur `insufficient_capabilities_for_transfer`); les E2E POS restent incomplets; la présence de toutes les colonnes de migration 0150 en production n’est pas vérifiée; les flux authentifiés et la revue visuelle complète ne sont pas passés sur le Preview. Il faut aussi confirmer le SHA prod courant avant une promotion. XCTest stable et un build iOS/TestFlight aligné sont encore requis pour livrer le natif. L’historique Supabase distant est divergent; 0150 n’a été appliquée qu’au staging par DDL ciblé.
