...
- [ ] Web : examiner/isoler les changements locaux, produire un commit de candidate vérifiée, redéployer un Preview correspondant à son SHA, refaire les parcours et la revue visuelle, puis seulement promouvoir la version en production et contrôler les domaines publics.
...
- **Mise à jour Sentry Vercel** : les sept variables Sentry obsolètes (`SENTRY_AUTH_TOKEN`, clés/destinations OTLP/log drain, ancien DSN, organisation et projet) ont été retirées du projet. `NEXT_PUBLIC_MINERVA_SENTRY_DSN` est configurée en Production, Preview et Development pour `minerva-s5m/minerva-flow-web`; le nouveau `SENTRY_AUTH_TOKEN` est stocké comme secret en Production et Preview. L’authentification Sentry CLI a réussi. `next.config.ts` n’active téléversements de source maps, création de releases et moniteurs Vercel que si ce secret existe; la build Vercel suivante doit encore confirmer les permissions de téléversement. Le jeton ayant été transmis dans le chat doit être tourné après validation. La ressource Marketplace existe toujours sans projet lié.

- Dernier changement applicatif de la candidate `2.48.0` : commit `94776e2` sur `release/2.48.0`. Vercel Preview `dpl_71oMfaUwtuDU2ynA9ncvV5ojX8Hx`, `READY`; le SHA exact `94776e25acbb552036022722aeea2ced617071fd` est confirmé par les métadonnées Vercel. L’alias de branche est `https://minerva-flow-git-release-2480-endsi3gs-projects.vercel.app`. Les chemins du Preview sont protégés par Vercel; aucun E2E authentifié n’y a été lancé puisque son environnement Supabase pointe vers la production. Le dernier déploiement `main` de production reste au commit `f969e15`.
...
- **Pas de promotion production, tag de version, GitHub Release ni courriel utilisateur.** Restent la validation Checkout/réessai avec un compte Connect test actif, E2E POS, XCTest sur runner stable, revue manuelle des écrans et build iOS aligné. L’historique Supabase distant est horodaté et divergent des migrations locales : la 0150 a été appliquée au staging par SQL ciblé, sans `db push`; réconcilier la piste d’historique avant la prochaine livraison automatisée. Le compte fourni est un secret de test partagé dans la conversation : le renouveler après vérification. Le lien Paramètres est maintenant disponible aux propriétaires et gestionnaires pour rendre l’onboarding Connect accessible; les autres routes produit restent soumises à l’allowlist.
- **Sentry — 2026-09-25** : le code web envoie vers `minerva-s5m/minerva-flow-web` avec `NEXT_PUBLIC_MINERVA_SENTRY_DSN` distinct de la variable Vercel existante; iOS envoie vers le projet `minerva-flow-ios`. Un événement synthétique non personnel a été confirmé dans le projet web; le DSN iOS compile, mais la réception native reste à vérifier. PII, en-têtes, cookies, corps, paramètres d’URL, données DB, journaux Sentry et Replay sont désactivés; traces à 10 % en production. Build web Webpack (328 routes), TypeScript, ESLint ciblé et compilation iOS Simulator réussis. Téléversement de source maps, création de release au build et moniteurs Vercel restent désactivés jusqu’à création d’un jeton limité à la bonne organisation. La ressource Marketplace `sentry-copper-notebook` (forfait Developer gratuit) existe, mais la liaison à `minerva-flow` n’a pas fini car `NEXT_PUBLIC_SENTRY_DSN` est déjà présent; ne pas relancer `integration add` (cela crée une autre ressource). Les alertes projet créées via MCP n’ont pas pu être relues, même par l’endpoint alternatif; leur état est à confirmer dans Sentry.
