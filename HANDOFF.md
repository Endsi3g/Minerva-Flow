...
- [x] Web : candidate commitée et poussée; Preview courant `dpl_BZLJBoAYbFbcEmv4pSx3qfdXsSJG` (`d5f9b76`) READY. Contrôles HTTP et Playwright visuel login desktop/mobile terminés; captures `docs/screenshots/preview-login-2026-09-25-{desktop,mobile}.png`.
...
- **Candidate web `2.48.0`** : code applicatif `20197a4c85f3aed215c6cd12ce69004b9ab70a4b`; commit documentaire `d5f9b76c0ecb01aafe65af40c3ef0f9c7cb673cf`. Preview courant `dpl_BZLJBoAYbFbcEmv4pSx3qfdXsSJG`, `READY`; alias de branche `https://minerva-flow-git-release-2480-endsi3gs-projects.vercel.app`. Build/typecheck/source maps Sentry confirmés. Smoke login, inscription, vie privée et conditions : 200; pages owner sans session redirigent vers login. Visuellement, login desktop (1440×900) et mobile (390×844) n’ont ni overflow horizontal ni erreur JavaScript; `/en/login` affiche toutefois du texte français, à vérifier. Production actuelle inspectée en lecture seule : `dpl_A35dLpmKawVPfxhp9rWEWSiVcRLc`, branche `main`, commit `f969e15`.
...
- Le configurateur de formats tarifés pour les propriétaires et la sélection correspondante dans le panier client sont intégrés au code web/iOS et consignés au changelog. La migration 0151 n’est pas appliquée en production : `menu_items.price_options` est absente; ne pas annoncer ni activer ce parcours avant l’application contrôlée de cette migration. Ces changements sont commités séparément de la candidate 2.48.0.
