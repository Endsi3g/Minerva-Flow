...
- **Production : NOT READY — aucune promotion, migration, tag, release GitHub ni courriel utilisateur.** Vérification Supabase de production en lecture seule : `restaurants.delivery_enabled` et `orders.delivery_address` existent, mais les quatre champs 0150 (`stripe_connect_account_api_version`, `stripe_connect_transfers_status`, `stripe_connect_recipient_payouts_status`, `stripe_connect_requirements_due_count`) et `menu_items.price_options` (0151) sont absents. Le checkout Stripe Connect complet reste non validé (`insufficient_capabilities_for_transfer`); POS E2E incomplet; le staging E2E courant a échoué avant assertion métier. Les migrations 0150 et 0151 n’ont pas été appliquées en production; l’historique distant est divergent. Ne pas promouvoir avant résolution/validation de ces blocages.
...
- Le configurateur de formats tarifés pour les propriétaires et la sélection correspondante dans le panier client sont intégrés au code web/iOS au commit `6621c38` et consignés au changelog. La migration 0151 n’est pas appliquée en production : `menu_items.price_options` est absente; ne pas annoncer ni activer ce parcours avant l’application contrôlée de cette migration.
